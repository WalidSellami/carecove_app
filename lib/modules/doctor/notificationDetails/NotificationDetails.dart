...
class NotificationDetails extends StatelessWidget {
  final PatientClaimData patientClaim;
  const NotificationDetails({super.key, required this.patientClaim});
      ...
      builder: (context , state) {
        ...
        return BlocConsumer<AppCubit , AppStates>(
          listener: (context , state) {
            ...
            if(state is SuccessRemovePatientClaimAppState) {
          ...
          builder: (context , state) {
            ...
            return Scaffold(
              appBar: defaultAppBar(
                context: context,
                title: '${patientClaim.patient?.user?.name}',
                actions: [
                  if(checkCubit.hasInternet)
                    ConditionalBuilder(
                    condition: state is! LoadingRemovePatientClaimAppState,
                    builder: (context) => IconButton(
                      onPressed: () {
                       showAlert(context, patientClaim.patientClaimId);
                      },
                      icon: Icon(
                        Icons.close_rounded,
                        size: 26.0,
            ...
            actions: [
              ...
              TextButton(
                onPressed: () {
                  Navigator.pop(context);
                  showLoading(context);
                  AppCubit.get(context).removePatientClaim(idClaim: id);
                },
                child: Text(
                  'Yes',
                  style: TextStyle(
